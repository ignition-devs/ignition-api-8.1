...
def httpClient(timeout=60000, bypass_cert_validation=True, username=None,
    ...
    calling a method (such as get(), post()) on the JythonHttpClient
    ...
    Args:
        ...
        bypass_cert_validation (bool): A boolean indicating whether
            the client should attempt to validate the certificates of
            remote servers, if connecting via HTTPS/SSL. Defaults to
            True. Optional.
        username (str): A string indicating the username to use for
            authentication if the remote server requests
            authentication; specifically, by responding with a
            WWW-Authenticate or Proxy-Authenticate header. Only
            supports Basic authentication. If username is specified
            but not password, an empty string will be used for the
            password in the Basic Authentication response. Defaults to
            None. Optional.
        ...
        cookie_policy (str): A string representing this client's
            cookie policy. Accepts values "ACCEPT_ALL", "ACCEPT_NONE",
            and "ACCEPT_ORIGINAL_SERVER". Defaults to
    ...
    Returns:
        JythonHttpClient: An object wrapped around an instance of
            Java's HttpClient class. The httpClient object has methods
            that can be called to execute HTTP requests against a
            server.
    """
