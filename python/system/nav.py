...
def getCurrentWindow():
    """Returns the path of the current "main screen" window, which is defined
    as the maximized window. With the Typical Navigation Strategy, there is
    only ever one maximized window at a time.

    Eeturns:
        str: The path of the current "main screen" window - the maximized
            window.
    """
...
def swapWindow(swapFromPath, swapToPath, params=None):
    ...
    and another is opened and takes its place - assuming its size, floating
    state, and maximization state. This gives a seamless transition - one
    window seems to simply turn into another.

    Args:
        swapFromPath (str): The path of the window to swap from. Must be a
